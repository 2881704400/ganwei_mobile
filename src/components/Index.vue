<template>
  <div class="gw-index">
    <header class="header">
       <statusbar :projectName = "projectName" @onfoldAside="foldAside"></statusbar>
    </header>
    <div class="main">
      <aside class="nav-left" :class="[isFold ? 'close' : 'open']">
        <nav class="nav-list">
            <ul>
              <li class="borderBottom"><a href="#"><i class="iconfont">&#xe62e;</i>{{$store.state.loginMsg}}</a></li>
              <li class="borderBottom"><a href="#"><i class="ivu-icon ivu-icon-information-circled"></i>关于</a></li>
              <li class="borderBottom"><a href="#"><i class="iconfont">&#xe641;</i>注销</a></li>
            </ul>
        </nav>
      </aside>
      <section class="main-body" :class="{home : $store.state.curPage.isHome}">
        <router-view class="router-page" v-if="loadCompleted" :rootSave="navList" @updateNavState="doNavState"></router-view>
      </section>
    </div>
    <footer class="footer">
         <toolbar :menuSave="menuList"></toolbar>
    </footer>
  </div>
</template>
